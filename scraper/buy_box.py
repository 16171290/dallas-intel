...
2. **Missing data is not exclusion.** If a record has no ``dcad_market_value``
   (because address-resolution failed), the price check is skipped —
   we don't exclude on the basis of "we don't know." The same applies
   to missing ZIP. Exclusion happens only when we have data and the
...
class BuyBox:
    ...
    def describe(self) -> str:
        ...
        if self.zip_allowlist:
            bits.append(f"zip∈{{{len(self.zip_allowlist)}}}")
        if self.zip_denylist:
            bits.append(f"zip∉{{{len(self.zip_denylist)}}}")
    ...
    def evaluate(self, record: dict) -> tuple[bool, list[str]]:
...
def annotate_records(
    ...
    If ``buy_box`` is None, uses ``BuyBox.from_env()``.
    """
    if buy_box is None:
    ...
    for rec in records_list:
        in_bb, reasons = buy_box.evaluate(rec)
