...
def find_dcad_zip() -> Optional[Path]:
    ...
    Resolution order:
      ...
      3. ``scraper.dcad_bulk.fetch_dcad_zip()`` — download if neither cache hits
                                                   (slow, requires network)

    We return ``None`` when no ZIP is reachable and the caller decides whether
...
def fetch_dcad_zip_or_die() -> Path:
    """Locate a DCAD ZIP via find_dcad_zip; if missing, attempt the canonical
    download via scraper.dcad_bulk.fetch_dcad_zip(). Exits with a clear
    message if no ZIP is reachable."""
    ...
    try:
        ...
        sys.exit(
            ...
            f"({type(e).__name__}: {e}). "
            f"Looked in: data/, {config.DCAD_CACHE_DIR if 'config' in dir() else '~/.dcad_cache'}. "
            f"Run main.py once to populate the cache, or set the DCAD_ZIP_URL env var."
        )
